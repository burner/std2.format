...
/*
   ...
   Source: $(PHOBOSSRC std2.format/internal/write.d)
 */
...
@safe unittest
{
    ...
    import std2.format : formatValue;
...
if (isInputRange!T)
{
    ...
    import std2.format : FormatException, formatValue, NoOpSink;
...
{
    import std2.format : formattedWrite;
    import std.range.primitives : put;
...
{
    import std2.format : enforceFmt, formatValue;
    import std.range.primitives : put;
...
@safe unittest
{
    ...
    s = format("%s", c2);
    assert(s == "immutable(std2.format.internal.write.C)");

    const(C) c3 = new C();
    s = format("%s", c3);
    assert(s == "const(std2.format.internal.write.C)");

    shared(C) c4 = new C();
    s = format("%s", c4);
    assert(s == "shared(std2.format.internal.write.C)");
}
...
{
    import std.array : appender;
    import std2.format.spec : singleSpec;
...
private void formatTest(T)(string fmt, T val, string[] expected, size_t ln = __LINE__, string fn = __FILE__) @safe
{
    ...
    import std.conv : text;
    import std.exception : enforce;
    import std2.format.write : formattedWrite;
